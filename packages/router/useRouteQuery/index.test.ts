...
describe('useRouteQuery', () => {
  ...
  it('should change the value when the route changes', () => {
    ...
    const page: Ref<any> = useRouteQuery('page', '1', { route, router })
  ...
  it('should allow custom serializer', async () => {
    let route = getRoute()
    const router = { replace: (r: any) => route = r } as any

    const dateFoo = new Date('2024-01-01T00:00:00.000Z')
    const dateBar = new Date('2024-02-02T00:00:00.000Z')

    const date = useRouteQuery('timestamp', dateFoo.getTime(), {
      serializer: {
        read: v => new Date(v),
    ...
    expect(date.value).toEqual(dateFoo)

    date.value = dateBar

    await nextTick()
    expect(date.value).toEqual(dateBar)
    expect(route.query.timestamp).toBe(dateBar.toISOString())

    const dateBaz = new Date('2024-03-03T00:00:00.000Z')
    route.query = {
      timestamp: dateBaz.getTime(),
    }

    expect(date.value).toBe(dateBaz)
  })

  it('should allow custom serializer2', async () => {
    let route = getRoute()
    const router = { replace: (r: any) => route = r } as any
    ...
    expect(date.value).toEqual(dateBaz)
  })
})
